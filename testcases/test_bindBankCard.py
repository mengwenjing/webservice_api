...
class TestBindCard(unittest.TestCase):
    ...
    testdata = DoExcel(case_file,'bindBankCard').read_data()

    @classmethod
    def setUpClass(cls):
        cls.excel = DoExcel(case_file, 'bindBankCard')
        cls.mysql = DoMysql()
    ...
    def testBindCard(self,case):
        ...
        if case.title == '正常发送验证码':
            ...
            try:
                ...
                self.excel.write_data(case.case_id + 1, str(res), 'failed')
                logger.error("实名认证失败，错误是：{}".format(e))
        elif case.title == '成功绑定银行卡':
            case.data = Context().replace(str(case.data))
            logger.info('执行用例是：{}，请求url是：{}，请求数据是：{}'.format(case.title, case.url, case.data))
            res = Request('bindBankCard').request(case.url, eval(case.data))
            try:
                self.assertEqual(res['retInfo'], eval(case.expected)['retInfo'])
                self.excel.write_data(case.case_id + 1, str(res), 'pass')
                logger.info('用例执行成功')
